...
class PlanarQuadrotor:
    ...
    def optimal_control(self, state, grad_value):
        ...
        coeff2 = ((grad_value[1]*jnp.cos(phi))/self.m) - ((grad_value[3])*self.l/self.Iyy)
        T1 = -1
        T2 = -1

        #the goal is to minimize total control input (T1 + T2)
        T2 = jnp.where(coeff1 > 0, self.min_thrust_per_prop, self.max_thrust_per_prop)
        T1 = jnp.where(coeff2 > 0, self.min_thrust_per_prop, self.max_thrust_per_prop)
...
def target_set(state):
    ...
    stateInd2 = jnp.where(state[1] >= -1, -1., 1.) #returns a negative is vY is feasible
    stateInd22 = jnp.where(state[1]  <= 1, -1., 1.) #returns a negative is vY is feasible
...
def envelope_set(state):
    ...
    y, v_y, phi, omega = state
    stateInd1 = jnp.where(state[0] >= 4., -1., 1.) #returns a negative if y is feasible
    stateInd11 = jnp.where(state[0] <= 7., -1., 1.) #returns a negative if y is feasible
    stateInd2 = jnp.where(state[1] >= -6, -1., 1.) #returns a negative is vY is feasible
    stateInd22 = jnp.where(state[1]  <= 6, -1., 1.) #returns a negative is vY is feasible

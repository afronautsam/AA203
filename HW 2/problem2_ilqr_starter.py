...
def ilqr(f, s0, s_goal, N, Q, R, Qf):
    ...
    for _ in range(max_iters):
        ...
        rangeList = np.arange(N+1)
        backOrder = np.sort(rangeList)[::-1] #descending order

        for k in backOrder: 
            if k == N: #terminal state 
                Qn = Qf
                V[k] = Qn 
                boldV[k] = np.matmul(Qn, (s_bar[k] - s_goal)) 
                regV[k] = 0.5*np.matmul(np.matmul((s_bar[k] - s_goal).T,Qn), (s_bar[k] - s_goal)) #maybe not? Change if needed

            else:
                cK = 0.5*np.matmul(np.matmul((s_bar[k] - s_goal).T, Q), (s_bar[k] - s_goal)) + np.matmul(np.matmul(u_bar[k].T, R), u_bar[k]) #maybe ditch the uK
                cxK = np.matmul(Q,(s_bar[k] - s_goal)) #should these be a sum?
                cuK = np.matmul(R, (u_bar[k])) #are my c values correct?
                cuuK = R
                cxxK = Q
                cuxK = 0 #analytically 

                Qk = cK + regV[k + 1]
                QxK = cxK + np.matmul(A[k].T, boldV[k + 1])
                QuK = cuK + np.matmul(B[k].T, boldV[k + 1])
                QxxK = cxxK + np.matmul(np.matmul(A[k].T, V[k+1]), A[k])
                QuuK = cuuK + np.matmul(np.matmul(B[k].T, V[k + 1]), B[k])

                QuxK = cuxK + np.matmul(np.matmul(B[k].T, V[k + 1]), A[k])
                
                l[k] = np.matmul(-np.linalg.inv(QuuK), QuK)
                L[k] = np.matmul(-np.linalg.inv(QuuK), QuxK)
                ...
                regV[k] = Qk - 0.5*np.matmul(np.matmul(l[k].T, QuuK), l[k])
                boldV[k] = QxK - np.matmul(np.matmul(L[k].T, QuuK), l[k])
                V[k] = QxxK - np.matmul(np.matmul(L[k].T, QuuK), L[k])


        ## Forward Pass
        for k in range(N):
            dU[k] = l[k] + np.matmul(L[k],(s[k] - s_bar[k]))
            u[k] = u[k] + dU[k]
            s[k+1] = f(s[k], u[k])
            #dS[k+1] = A[k]@dS[k] + B[k]@dU[k] #updating delta S 'dS
...
if __name__ == "__main__":
    ...
    for k in range(N):
        if simulate_continuous_time_dynamics:
            ...
            u[k] = u_bar[k] + l[k] + np.matmul(L[k], (s[k] - s_bar[k]))
            ###################################################################
    ...
    ani.save('cartpole_swingup.gif', writer='ffmpeg')
    plt.show()

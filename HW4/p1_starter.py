...
def finish_episode():
    ...
    for (log_prob, value), R in zip(saved_actions, returns):
        ...
        # and from advantage
        polLoss = -log_prob*advantage #negative forces optimizer.step to be positive
        
        # append this to policy_losses
        policy_losses.append(polLoss)
        
        # TODO calculate critic (value) loss
        value_losses.append(F.mse_loss(value, torch.tensor([R]))) #make this an L2 loss or MSE loss
    # reset gradients
    optimizer.zero_grad()
    ...
    for i_episode in count(1):
        ...
        for t in range(1, 2500):
            ...
            if done:
                ...
                episodes_holder.append(i_episode)
                break
        ...
        if running_reward > 200:
            ...
            fig,ax = plt.subplots()
            ax.plot(episodes_holder,episodic_rewards)
            plt.show()
            break
